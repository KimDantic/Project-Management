...
full_name_filter = st.sidebar.multiselect("Filter by Full Name", options=full_name_options, default=["All Users"])
...
tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
    "Overview", "📊 Summary", "📈 Main Visualizations", "🏆 Top and Bottom Users ",
    "☁️ Word Cloud", "👤 User Drilldown", "Extra Insights"
])
...
with tab3:
    ...
    if time_group == "Yearly":
        ...
        # Format as "Jan 5, 2025" for the start of each week
    ...
    if not filtered_data.empty and 'minutes' in filtered_data.columns:
        ...
        if 'user_first_name' in filtered_data.columns and 'user_achievements_count' in filtered_data.columns:
            achievement_df = filtered_data.groupby('user_first_name')['user_achievements_count'].sum().reset_index()
        fig_ach = px.bar(
            achievement_df,
            x='user_first_name',
            y='user_achievements_count',
            title="Achievements per User",
            color='user_achievements_count',
            color_continuous_scale='Greens',
        labels={'user_first_name': 'User', 'user_achievements_count': 'Total Achievements'}
        )
